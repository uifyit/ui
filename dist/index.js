...
async function initCommand() {
    ...
    const filesToCopy = [
        ...
        },
        {
            ...
            to: path.resolve(process.cwd(), "src", "uifyit", "utils", "cn", "index.ts"),
        },
    ];
    ...
    try {
        for (const file of filesToCopy) {
            ...
            console.log(chalk.green(`✔ Copied ${path.basename(file.from)} to ${file.to}`));
...
async function addCommand() {
    ...
    const allExportLines = new Set(existingExports.split("\n").filter(Boolean).concat(Array.from(exportLines)));
